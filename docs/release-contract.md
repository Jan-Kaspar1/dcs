...
stage's managed run-state carryover leg, which proves the managed
alarm kinds' checkpointed run state carries across a takeover on the
deployed pair: a per-pump fault alarm held `out_of_service` through
its wired `oos` point and tripped suppressed, the shelvable alarm
shelved mid-run through its writable journaled shelve point, the
documented `demote`/`promote` landing inside the declared
`max_shelve_ticks` bound, the promoted peer asserting `shelved`
stands and releases at the tick the continued countdown expires —
never a bound restarted at the switch — `out_of_service` standing
with evaluation held, and both durable journals' ordered records
continuous across the switch — and the
`upgrade` stage, which repins
...
| `alarm-rationalization-failed` | The pair contract's alarm-rationalization leg did not hold: a peer's served `GET /signals` components section did not carry every managed alarm instance's declared `rationalization` block verbatim — the record dropped or a declared field rewritten — the served `GET /snapshot` parameters section did not report each alarm's declared `priority`/`class`/`response_ticks` live, the record diverged across the documented `demote`/`promote` switch, or the pair was not left in its declared roles. Reported by the reference plant's `ci/check.sh`, with the leg's `alarm-rationalization: …` evidence lines on stderr. |
| `alarm-rationalization-nondeterministic` | Two passes of the alarm-rationalization leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `claim-fencing-failed` | The pair contract's claim-fencing leg did not hold: with a field-owning peer holding the spawned plant's writer claim, a dedicated third sim-net attachment's `write` or `step` was not refused with the named fencing failure — or the same mutations through the shipped `dcs-plant-ctl` did not exit nonzero — or the field owner's writes or `active` role were disturbed; or the claim lifecycle verbs answered off-contract — a foreign token's `ensure_writer` not refused `fenced`, the owner's token not answering `claimed_shared`, a write under the shared hold refused, `release_writer` dropping more than the caller's hold, or a holder-of-nothing's release not a harmless `done`; or a rogue `claim_writer` answered neither the refusal nor the unconditional preempt the contract admits — a preempt that left the superseded owner serving `active` past its fenced scans, never journaled `field_claim_lost`, or walked a role transition other than `active → demoting → standby`; or a receipted write admitted in the preempt window settled anything but `superseded` on either peer's receipt log while the demoted image, the tracking peer's adopted checkpoint, or the re-promoted line carried the written value — a superseded command that landed; or the restored pair did not settle its launch roles with the claim's owner standing. Reported by the reference plant's `ci/check.sh`, with the leg's `claim-fencing: …` evidence lines on stderr. |
| `claim-fencing-nondeterministic` | Two passes of the claim-fencing leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
