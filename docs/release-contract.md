...
| `claim-reclaim-failed` | The pair contract's claim-reclaim leg did not hold: with the manifest-declared pair converged in driven mode on the released images, a dedicated plant-socket attachment's `claim_writer` under a foreign owner token did not preempt the standing field claim unconditionally, a third-party mutation probe under the preempted claim did not stay `fenced` naming the induction token, the superseded field owner did not demote in place — its monitor stopping answering, or its role walk diverging from `active → demoting → standby` — its served journal did not gain `field_claim_lost` attributed to the induction token, the loss-marked ex-owner's bound conditional re-grant took a different-owner claim during the held window — leaving converged standby, letting the standing claim move off the induction token, or letting a foreign write land — the preemptor's `release_writer` hand-back was refused, the released claim never re-armed to the standing owner — the loss-marked reclaim never walking the ex-owner `standby → promoting → active` unattended, the re-seated claim not fencing third-party mutations naming the launch owner or not answering `claimed_shared` under the owner's token — the re-seated claim fenced the owner's own writes, the unattended walk back to active left no journaled record, the tracking peer journaled a fencing loss or role change it never owned, or the pair did not reconverge to one active plus one tracking standby with the launch roles restored. The leg reports `inconclusive` — never this failure — when the pinned release predates the contract: the launched active recording no owner-token claim line, a claim verb answering `invalid_request`, or a fencing verdict naming no standing owner. Reported by the reference plant's `ci/check.sh`, with the leg's `claim-reclaim: …` evidence lines on stderr. |
| `claim-reclaim-nondeterministic` | Two passes of the claim-reclaim leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
