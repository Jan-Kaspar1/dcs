...
| `schema-drift` | `dcs-model schema`, `dcs-model interface-schema`, `dcs-model deploy-schema`, or `dcs-plant-server --dynamics-schema` at the pinned rev did not emit the release record's recorded artifact bytes (`plant-model.schema.json` / `block-interfaces.schema.json` / `deploy-manifest.schema.json` / `dynamics.schema.json`) — the emitted schema drifted from what the release record pins. Reported by the reference plant's `ci/check.sh`. |
| `schema-mismatch` | A document failed its recorded schema artifact's structural conformance — a required field absent or mistyped, a vocabulary outside its `enum`/`const`, or an undeclared field under `additionalProperties: false`: the driven run's `GET /schema` document against `block-interfaces.schema.json`, the checked-in deployment manifest against `deploy-manifest.schema.json`, or the checked-in dynamics document against `dynamics.schema.json`. Reported by the reference plant's `ci/check.sh`. |
| `schema-mismatch-nondeterministic` | Two passes of a consumer-document schema-screening leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
...
| `attribution-failed` | The pair contract's attributed role-switch leg did not hold: an attributed `POST /demote`/`POST /promote` carrying the declared `ci` actor against the converged pair did not journal each peer's switch `role_changed` entries with `origin: "request"` and the declared actor — attributed at the request's own scan boundary for the request transition and the following tick for the settle, ordered after the pre-request record — on either peer's durable `--journal-file` in both switch directions, the tracking peer's checkpoint-adopted journal did not show the same attributed record (the receipted command settled on the promoted owner adopting without the declared actor carried over the checkpoint), the armed standby's self-promotion at the declared `failover_budget` miss boundary did not journal `origin: "failover"` with no operator actor — distinguishable from any operator request — or the pair was not left in its launch roles. Reported by the reference plant's `ci/check.sh`, with the leg's `attribution: …` evidence lines on stderr. |
| `attribution-nondeterministic` | Two passes of the attributed role-switch leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
...
| `ack-edge-failed` | The pair contract's consumed-edge acknowledgment lifecycle leg did not hold: with the declared pair converged in driven mode, the field-driven `p101-moisture` contact did not annunciate the managed alarm's `alarm`/`unacknowledged` with journaled `point_changed` evidence, the receipted `ack` press did not settle `applied` under the leg's actor or consumed the false→true edge other than exactly once — the latch not clearing while `alarm` kept reporting the standing condition and the input serving held `true` — the held level pre-acknowledged the fresh trip's re-latch, the `demote`/`promote` switch under the held input re-consumed the stale edge or dropped the latch, the promoted peer's served checkpoint did not carry the adopted `ack`/`unacknowledged` states, the `ack=false` release submitted to the demoted peer did not answer its journaled `not_active` refusal — or cleared the held input or the latch anyway — the held press did not settle `applied` as a journaled no-op or re-consumed anyway, the late release did not re-arm the edge, the recovered press did not consume and clear the latch, the acknowledged state did not ride the restore switch back to the manifest's declared roles, a driven input was not restored, the peers' adopted receipt logs diverged, or a peer's durable journal did not carry the lifecycle's attributed transitions and settlements in `seq` order with the served journal answering the same record. The leg reports `inconclusive` — never this failure — when the pinned release's served checkpoint predates the consumed-edge contract. Reported by the reference plant's `ci/check.sh`, with the leg's `ack-edge-lifecycle: …` evidence lines on stderr. |
| `ack-edge-lifecycle-nondeterministic` | Two passes of the consumed-edge acknowledgment lifecycle leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `invoke-args-failed` | The pair contract's invoke argument-schema leg did not hold: with the declared pair converged in driven mode, `dcs-ctl invoke`'s `advance bogus=1` — an argument name the sequencer's declared `request [{name: count, kind: int}]` does not carry — did not answer the named `rejected{unknown_argument}` verdict naming component, command, and offending argument, the refusal was not terminal on the wire while mirrored in the served receipt log and journaled `command_settled` exactly once at the run's standing tick with no scan driven, the refused submission queued or applied (`command_queue.depth` moved, the sequencer's `step`/`done` left their quiet baseline, or the tracking peer's adopted receipt log or journal disagreed), `advance` with `count` omitted did not answer `accepted` and settle `applied` with the kind's absent-argument default moving the table exactly one step on both peers' adopted audit, the `count=two` malformed-value probe did not fail its declared-kind parse inside `dcs-ctl` before any submission — nonzero exit naming `invalid value`, no receipt, no receipt-log or journal record — so the three argument-handling classes stayed named distinctly, or the pair was not left in its launch roles. A pinned release predating the invoke-admission contract — the undeclared name admitted rather than refused, the served registry carrying no `advance` request schema, or the released `dcs-ctl` refusing the probe before submission — reports `invoke-args-inconclusive` rather than failing machinery it lacks. Reported by the reference plant's `ci/check.sh`, with the leg's `invoke-args: …` evidence lines on stderr. |
| `invoke-args-nondeterministic` | Two passes of the invoke argument-schema leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
...
| `rolling-upgrade-failed` | The pair contract's rolling controller-upgrade leg did not hold: the manifest-declared pair did not launch and converge `tracking` on the recorded upgrade-from release's `dcs-controller` beside the pinned release's `dcs-plant-server`, the stopped standby did not relaunch on the pinned binary resuming at its persisted tick and reconverge `tracking` inside the declared window while the surviving owner's field writes continued, the promoted pinned peer's writes did not land with the demoted predecessor peer reconverged `tracking` on its checkpoint stream, either side's receipted command did not settle `applied` exactly once into both peers' adopted log and served journal, the second peer's roll to the pinned binary did not reconverge the all-pinned pair to one `active` plus one `tracking` standby, the plant's step record named a driven window the field stood unowned, the durable journal files did not carry the run's boundaries and `seq` chain or each command's `command_settled` exactly once, a declared state file did not checkpoint the run's final tick under the manifest's model fingerprint, or the pair was not left in its launch roles. A named checkpoint-format or model-format refusal across the recorded release boundary — a predecessor member that cannot run the deployment, a pinned relaunch refused reading the predecessor's artifacts, or a peer's `degraded` report naming the version or fingerprint refusal — reports `rolling-upgrade-inconclusive`, the recorded incompatibility, rather than a false pass or this failure. Reported by the reference plant's `ci/check.sh`, with the leg's `rolling-upgrade: …` evidence lines on stderr. |
| `rolling-upgrade-nondeterministic` | Two passes of the rolling controller-upgrade leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `nonfinite-refusal-failed` | The pair contract's nonfinite write and step refusal leg did not hold: with the declared pair converged in driven mode, a plant-protocol attachment sharing the field owner's standing claim — `ensure_writer` under the launched active's recorded owner token — did not meet the value contract's named refusals for its protocol-legal non-finite payloads: a `write` carrying `{"float":1e999}` or `{"float":-1e999}` answered `done` or an off-contract error rather than `invalid_request` or `io.invalid_value`, a `step` carrying `"dt":1e999` or `"dt":-1` answered `stepped` or an off-contract error rather than `invalid_request`, the refused payloads left the driven point or the whole served census moved or carrying a non-finite float — the `{"float":null}`-family frame the shipped `dcs-plant-ctl`'s own `read`/`list` decode cannot print — the released `dcs-plant-ctl` did not refuse its own non-finite or out-of-range spellings by name, a legitimate finite write and step under the same claim did not answer `done`/`stepped` with the plant tick advancing, the driven point's baseline did not restore, the converged or post-probe served snapshots carried a non-finite sample on either peer, the pair's launch roles moved, or the field owner's `io_health` counted the refused payloads. The leg reports `inconclusive` — never this failure — when the pinned release predates the contract: the launched active recording no owner-token claim line, `ensure_writer` unanswered or refused under the pinned token, the census serving no finite float field input, the step path not live for a `dt:0` probe, or a probe meeting the fencing verdict rather than the value contract. Reported by the reference plant's `ci/check.sh`, with the leg's `nonfinite-refusal: …` evidence lines on stderr. |
| `nonfinite-refusal-nondeterministic` | Two passes of the nonfinite write and step refusal leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `driver-recovery-failed` | The pair contract's remote-driver recovery leg did not hold: with the manifest-declared pair converged in driven mode on the released images, the field owner's interrupted plant link — the stopped `dcs-plant-server` lifetime — did not surface the served backend diagnostics `disconnected` carrying the named `last_error` while io_health's consecutive-failure streak counted and the boundary fault stayed recorded, the first successful exchange after the link's restore did not clear the backend's `last_error` and reset the streak while the cumulative per-direction counters and the recorded fault kept the outage's history, the served tick rewound or stalled across a cycle, the tracking peer's backend never re-attached, the pair's launch roles moved, or the two staged interruptions' digests diverged. A served `io_health` carrying no backend driver diagnostics, a link that never presented the healthy baseline, or a link-staging lever that cannot land — the stop never completing, the restarted plant never serving the interrupted address — reports `driver-recovery-inconclusive` rather than this failure. Reported by the reference plant's `ci/check.sh`, with the leg's `driver-recovery: …` evidence lines on stderr. |
| `driver-recovery-nondeterministic` | Two passes of the remote-driver recovery leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `command-overflow-order-failed` | The pair contract's command-overflow submission-order leg did not hold: with the declared pair converged in driven mode, a labeled `POST /command` flood past the command lane's declared queue bound — `LANE_QUEUE_DEPTH` plus twice the field owner's served `command_queue.capacity`, landed behind a deliberately half-sent first submission pinning the lane's one worker — did not mint its receipts in submission order on the field owner's live receipt mirror, an admitted submission inside the bound did not answer `accepted`, a submission past the queue's declared bound did not answer the named `queue_full` rejection carrying the bound and the refused target, a minted or answered receipt did not echo its own submission, the held flood moved the run's tick, disturbed the served reads, or moved a role, the first drain scan did not land the owner's tick exactly one boundary on, the adopted receipt logs diverged or did not keep the flood's mint order settling the admitted half `applied` while the overflow stayed `queue_full`, both peers' `command_queue` sections did not report the flood's admission metrics identically — every submission an `attempt`, the over-bound count `full_rejections`, `high_water` at the bound, `depth` drained — or the pair was not left in its launch roles. The leg reports `inconclusive` — never this failure — when the pinned release predates the contract: a submission answering no receipted frame, or the served receipt log minting the labeled flood out of submission order — the pre-contract refuse-path shape. Reported by the reference plant's `ci/check.sh`, with the leg's `command-overflow-order: …` evidence lines on stderr. |
| `command-overflow-order-nondeterministic` | Two passes of the command-overflow submission-order leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `state-file-sink-isolation-failed` | The pair contract's state-file sink-isolation leg did not hold: with the manifest-declared pair converged in driven mode and the field owner's state-file sink impeded — a reader-less FIFO staged at the sink's write-then-rename temporary path, the drain writer's `open()` blocking inside the mount while captures queue behind the bounded handoff — the driven `POST /scan` batch's served tick did not advance across the impeded window inside the declared bound, the field owner's served `io_health` counters moved, `publication.state_sink` never reported the named `lagging` state with `depth` bounded inside `capacity` — the sink instead reporting `failed`, an unnamed state, a queue accounting nothing or unboundedly, or `lost` captures before any write failed — a serving-lane read did not answer inside its bound while the sink stood stalled, the batch's durability-attesting answer completed while the file could not have caught up or did not answer `200` at the batch's tick after the restore, a fresh publication after the restore did not report `healthy` with the queue empty and `lost` accounting nothing, the durable state file did not parse to the run's scanned tick — checkpointing never resuming — or the pair did not stand in its launch roles, the field owner `active` and its standby `tracking`. The leg reports `inconclusive` — never this failure — where the consumer harness admits no impediment lever: the manifest's field owner declaring no `state_file`, the runner unable to stage the FIFO, the staged node refused or never releasing, or the served revision predating the `state_sink`/`io_health` surface. Reported by the reference plant's `ci/check.sh`, with the leg's `state-file-sink-isolation: …` evidence lines on stderr. |
| `state-file-sink-isolation-nondeterministic` | Two passes of the state-file sink-isolation leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `bare-point-restamp-failed` | The pair contract's bare-point restamp leg did not hold: with the manifest-declared pair converged in driven mode, the bare channel-bound field input the emitted model declares — a channel-bound `in` point no dynamics element output and no field-side loopback drives — did not keep its served stamp advancing with every landed plant step across the declared wait horizon past its freshness budget, the stamp idled past the declared budget, the non-bare contrast's served stamp diverged from the plant tick of the step that produced it — the bare-only re-stamp blurring the deadline-driven stamp's real evidence — a served stamp advanced under the paused sim tick, the resumed driven ticks did not advance both stamps together, or the pair's launch roles moved. The leg reports `inconclusive` — never this failure — when the emitted model declares no bare channel-bound field input or no driven contrast, when the pinned release predates the restamping contract — the bare stamp never moving across a stepped wait horizon — or when the field's own step record cannot be attributed. Reported by the reference plant's `ci/check.sh`, with the leg's `bare-point-restamp: …` evidence lines on stderr. |
| `bare-point-restamp-nondeterministic` | Two passes of the bare-point restamp leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
| `suspended-alias-audit-failed` | The pair contract's suspended-receipt alias-audit leg did not hold: with the manifest-declared pair converged in driven mode, a rogue attachment's preempted field claim fencing the still-reporting-active owner, and the suspended admission — a `POST /command` receipted `accepted` at the collision index under its own `(actor, reason)` identity — standing suspended across the fenced demote, the holder was stopped so the promoted peer's adopted receipt window stayed stale, and the identical command — same point, same value, a different admission's `(actor, reason)` — minted at the suspended admission's own absolute index on the promoted peer; after the holder's rejoin as a standby on its declared listen address and the covering adoption, the suspended admission did not journal its one named `rejected{superseded}` adjudication on its holder — a second settle or another outcome recorded — a settle for it landed on the promoted peer that never held it, the identical admission did not settle `applied` exactly once per peer on its own receipt index, either peer's declared durable journal carried a different record than its serving monitor, or the pair was not left in its launch roles — the duty controller `active`, its standby `tracking`. The leg reports `inconclusive` — never this failure — when the pinned release predates the contract: a checkpoint carrying no receipt window or admission counters, the declared `state_file`/`journal_file` persistence absent, the refused preemption lever, a served receipt dropping the declared `actor`/`reason` the carry test compares, the demote settling rather than suspending the admission, or the covering adoption leaving the suspended admission vanished unaudited or still suspended and unadjudicated — the adopted window's identical entry aliased it as carried, the pre-contract shape. Reported by the reference plant's `ci/check.sh`, with the leg's `suspended-alias-audit: …` evidence lines on stderr. |
| `suspended-alias-audit-nondeterministic` | Two passes of the suspended-receipt alias-audit leg produced different digests. Reported by the reference plant's `ci/check.sh`. |
