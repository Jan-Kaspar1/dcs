...
Publication caps the dependency-ready frontier at twenty. Open `agent:ready` tickets waiting on prerequisites do not consume that capacity; newly published tickets count only when their dependencies are closed.
...
`python3 scripts/merge_flow.py` prints a read-only measurement behind `status`'s rolling merge comparison, for use when the decline exceeds the 20% investigation threshold. It scans first-parent commits on the integration ref — worker clones are shallow, so run it against a full clone (`--repo-dir`) or replay captured output (`--git-log`) — resolves each merge to its issue through the squash-merge body, and reports per named window (`previous`, `current`; `--window-days`, `--now`): merge counts, dispatch-to-merge lead-time percentiles where the job ledger (`--state-db`, auto-detected from the install) supplies a reservation, the per-area breakdown via `dcs-task` metadata, repair incidence (WIP-preservation and mid-flight `origin/main` integrations inside merge bodies, plus ledger `repairs`/`attempt` counters), repairs and redispatches grouped by their ledger-recorded cause class (`merge-conflict`/`ci-failure`/`publish-error` repairs; `worker-failure`/`quota-requeue` redispatches — the supervisor writes one attributed `repair`/`redispatch` work-events row per repair or retry), and the open backlog's ready/blocked label share at report time. Repair rows also carry recorded detail where the detection site captured it — merge-conflict repairs persist the conflicted paths git reported and ci-failure repairs persist the failed check names — so each window ranks conflicting paths by incidence (path-less rows in a named unclassified bucket), names whether the conflict load concentrates on few paths or spreads, joins repairs to each issue's managed area, and attributes failing checks where the ledger row or the job's terminal check record exposes them. To attribute a decline, each window also reports the work-ledger flow: `reserved`/`retry-reserved` dispatch counts, `status:blocked` park counts classified into the bounded cause classes (`quota-kill`, `timeout`, `stall-kill`, `ci-failure`, `merge-conflict`, `publish-error`, `worker-failure`, `other`), `merged-and-closed` completions, and jobs still blocked at the window's end reconstructed by replaying each issue's event stream — so fewer dispatches, more parks, and exhausted supply are distinguishable from the report alone. A park's recorded `jobs.error` applies only to the job's terminal park; earlier parks classify from ledger context (the attempt's `invocation:` outcome, the resolving `redispatch` class, repair history). Per-window dependency-blocked supply is not reconstructible from the ledger and is reported as a stated limitation rather than fabricated. Issue inventory comes from `gh issue list --state all` unless `--issues`/`--no-issues` is given; work events come from the same state db unless `--events` is given; every input is injectable so the report is deterministic for tests and replays. It changes no dispatcher, worker, or CI behavior.
...
`rust-tests` is the fast required leg: the whole workspace suite — unit, integration, and doc tests under `--locked` — minus the four nested clean-target proofs. Those proofs run as the separate `rust-proofs` assembly gate, on every PR as a non-blocking early signal and unconditionally on every `main` commit and `v*` tag; `docs/release-contract.md` requires it green on a tagged commit before release. The recorded tradeoff (#905): a regression only a nested proof catches can reach `main` before its post-merge `rust-proofs` run reports — accepted because those legs recompile the released crates in clean scratch targets and kept the required `rust-tests` gate over the five-minute feedback budget on a cold runner (10m17s on #889 against a 200.6s warm WSL run). No test is skipped: the workspace leg excludes exactly the four proofs and the gate reruns each by name. Measured on the worker WSL box at the split's revision: workspace leg 168.4s cold / 130.4s warm, proofs leg 233.2s cold / 214.9s warm — the four proofs alone exceeded the whole required gate. Every cached Rust CI job reports its cache-hit flags and cache sizes, and every phase logs its elapsed seconds; ordinary PR feedback latency is the required checks' wall time.
