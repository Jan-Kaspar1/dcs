...
- #105 — descriptor-driven parameter editing on faceplates through `set_parameter`, with the applied tick or named rejection displayed (decision 34's interim surface; depends on #62, #82). **Done** — each declared parameter renders an edit control typed and range-labeled from its `ParameterDescriptor`, submitting `set_parameter` through the receipted path with the applied tick or named rejection landing on the parameter's row and in the journal pane.
- #106 — forced points badged and releasable on the page (decision 21's UI slice; depends on #62, #84). **Open** — blocked on #84's forcing contract, in flight.
...
- #84 — persistent forcing of writable field `In` points at `Uncertain(Substituted)` quality, released at the scan boundary, listed and journaled (decision 21; its #49 dependency has landed). **Open** — in flight; issue filed under M4; #106 is its page slice.
...
Inspect current main, open issues, and PRs before updating this plan. M4's remaining bulk is the page work — #105's parameter editing landed, #106 waits on #84's forcing contract (PR in flight), #107's health line landed in substance under #83 with its issue in flight, and #108's alarm pane consumes the two-flag model #109 landed. M5's lifecycle set runs on the shipped pair machinery: #65 is in flight, #87 is ready on the fingerprinted checkpoint #66 landed, and #114–#116 landed the revision diff, the restart-recovery state file (decision 35), and the full-stack demonstration. M6's open work is the #84 contract — the #109–#111 kinds landed along with #134's step sequencer and #135's second-order lag element. M7 is complete — #112 landed `dcs-build`, #113 landed `dcs-sim-bus`, and #136 landed the `dcs-model lint` tooling. M8's three implementation tickets (journal file, overview page, actor identity) are to be filed per decisions 36–39; the ready queue also carries #131 (Standby/StandbySync merge), #132 (current parameter values — decision 34's follow-up), and #133 (cross-version checkpoint negotiation). Keep between six and twenty ready issues only when that much independent work exists.
