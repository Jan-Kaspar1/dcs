...
The Rust workspace holds thirteen implemented crates — `dcs-core`, `dcs-model`, `dcs-runtime`, `dcs-sim`, `dcs-sim-net`, `dcs-sim-bus`, `dcs-blocks`, `dcs-build`, `dcs-monitor`, `dcs-assembly`, `dcs-controller`, `dcs-plant`, and `dcs-demo` — covering the shared contracts, the versioned plant model with the `dcs-model diff` revision view (#114), the deterministic executor with checkpoint/restore and the tracking standby, the simulated I/O backend with process elements and fault injection, the TCP-served shared simulated plant and remote driver plus the `dcs-plant-ctl` operator tool, the register-mapped simulated fieldbus and its `dcs-sim-bus-device` server (#113), the reusable component library with per-kind descriptors and the timer/counter/rate-limiter (#85), latching-alarm (#109), manual/auto-station and signal-filter (#110), and median-voter/totalizer (#111) kinds, the typed Rust composition seam emitting versioned plant-model documents (#112), model-driven assembly with the device-kind driver registry (#47) now serving `sim*`, `sim-tcp`, `sim-scripted` (#67), and `sim-bus` (#113), the paced controller binary with active/standby modes (#32), role machine, and scan-boundary promotion/demotion with role reporting (#46) plus Docker packaging, the standalone shared-plant server accepting a declared dynamics document (#81), HTTP+JSON monitoring with signal groups (#86), history, journal, descriptor-driven faceplates (#62), the pair view (#63), and I/O health (#83), the showcase plant model exercising the full library (#69) and the scripted full-stack demonstration (#116), and the end-to-end simulated tank loop. The integration guide (`docs/integration-guide.md`, #71) documents both extension seams. Decisions are recorded in `docs/architecture.md` (issues #4, #18, #33, #53, #89, #70, #104, #137).
...
- #105 — descriptor-driven parameter editing on faceplates through `set_parameter`, with the applied tick or named rejection displayed (decision 34's interim surface; depends on #62, #82). **Open** — in flight.
...
- #108 — an alarm summary pane driven by status-role ports and the journal (decision 33's flags; depends on #62). **Open** — consumes the two-flag model #109 landed; decision 37 confirms the flags are the lifecycle's extent.
...
- the page's operator slices land on the faceplate and contract bases: descriptor-driven parameter editing through `set_parameter` (#105 — in flight), forced-point badging and release (#106 — blocked on #84), I/O-health and overrun rendering (#107 — landed in substance under #83), and the alarm summary pane (#108 — open) — **open**.

### M5: Lifecycle and device integration — in progress

The redundant pair becomes a lifecycle platform: automatic promotion when the active is lost, checkpoint versioning for cross-build replacement, declared-behavior device kinds beyond `sim`, an integration guide for new component and device kinds, and in-service model revision through the pair. Decisions 27–30 record this milestone's approach (#70, this record); decisions 25 and 26 (recorded under #89) already supply the model-revision and divergence-gate semantics. Six layers are implemented — the scripted device kind (#67) proves the registered device seam, the integration guide (#71) documents it, the divergence gate (#88) promotes only standbys proven to track the field, checkpoint versioning (#66) lets a standby negotiate cross-build compatibility by name, `dcs-model diff` (#114) supplies the revision-review tooling, and the full-stack demonstration (#116) runs the whole system end to end; the lifecycle tickets (#65, #87, #115) remain open — #65 and #115 in flight.
...
- #115 — persist controller checkpoints to a file for restart recovery (depends on #21, #66). **Open** — PR in flight; decision 35 records its interaction with M8's journal file.
...
- a controller's checkpoints persist to a file and a restarted process recovers the run from one (#115) — **open** — PR in flight;
...
The monitoring surface grows from one loop's page into an operations surface. The transition journal gains a durable file-backed sink surviving controller restart — monitor-local tooling beside #115's checkpoint state file, distinct in purpose: the checkpoint resumes the run's state, the journal preserves the run's record (decision 35). The page aggregates every configured controller pair into a plant-wide overview, with pair aggregation staying page-side rather than becoming a served contract (decision 36). The two-flag alarm lifecycle #109 landed stands as the decided model — shelving, suppression, and classes are recorded deferrals that land as per-kind declarations when demanded (decision 37) — and operator commands carry a declared actor identity journaled through the settled receipt, the safety-related-constraints step the vision names (decision 38). Decisions 35–38 record the approach (#137, this record).

Ticket breakdown:

- #137 — this decision record and plan refresh (decisions 35–38). **Done with this change.**
- The file-backed journal sink: a configured path appends every journaled entry at the recorder's documented point, startup replay seeds the in-memory ring and continues `seq` numbering, and a checkpoint-resumed run keeps appending in the restored tick domain; point history stays volatile (decision 35). **Ticket to be filed.**
- The plant-wide overview page: repeated pair-naming URL parameters render one summary card per configured pair — active peer, roles and convergence, the unreachable-peer fault, the I/O-health line — each linking into the existing pair view; no served contract addition (decision 36). **Ticket to be filed.**
- Actor identity on the command path: `POST /command` accepts the optional declared `actor`, `CommandReceipt` carries it through serde-optionally, `CommandSettled` journals the attribution, and the page supplies the configured operator name (decision 38). **Ticket to be filed.**
- No ticket — the alarm lifecycle stands at two flags plus the writable `ack` point; #108's summary pane consumes `alarm`/`unacknowledged` under M4 (decision 37).

Done when:

- a controller configured with a journal file preserves the transition journal across a restart — replayed into the served ring with continuing `seq`s and a run-boundary marker separating process lifetimes — while point history re-accumulates, and a `--resume`d run appends to the same file in the restored tick domain (decision 35) — **open**;
- one page instance lists every configured controller pair with per-pair health and links into each pair's view, aggregating entirely page-side over the existing JSON endpoints with no new wire type (decision 36) — **open**;
- the alarm lifecycle stays at `alarm`/`unacknowledged` plus the writable `ack` point, with shelving, suppression, and classes recorded as per-kind extension seams rather than contract additions (decision 37) — **recorded**;
- a submitted command's declared actor rides the receipt into the journaled `CommandSettled` entry — an absent actor journaling as unattributed, never a rejection — and the page supplies the configured operator identity, with the declared-not-authenticated limit documented (decision 38) — **open**.

## Next planning pass

Inspect current main, open issues, and PRs before updating this plan. M4's remaining bulk is the page work — #105's parameter editing is in flight, #106 waits on #84's forcing contract (in flight), #107's health line landed in substance under #83, and #108's alarm pane consumes the two-flag model #109 landed. M5's lifecycle set runs on the shipped pair machinery: #65 and #115 are in flight, #87 is ready on the fingerprinted checkpoint #66 landed, and #114/#116 landed the revision diff and the full-stack demonstration. M6's open work is the #84 contract — the #109–#111 library kinds landed. M7 is complete — #112 landed `dcs-build` and #113 landed `dcs-sim-bus`. M8's three implementation tickets (journal file, overview page, actor identity) are to be filed per decisions 35–38; the ready queue also carries #131 (Standby/StandbySync merge), #132 (current parameter values — decision 34's follow-up), #133 (cross-version checkpoint negotiation), and the in-flight library and tooling slices #134–#136. Keep between six and twenty ready issues only when that much independent work exists.
