...
clear per-point faults on the shared simulated field. The field-claim
case attaches a third sim-net client on the same endpoint and drives
the writer-claim lifecycle against the standing claim — fenced
mutations, conditional and shared grants, releases, and a rogue
preemption — with ctx['owner_tokens'] carrying the rig's pinned
per-peer --owner-token values.
...
# each peer's --owner-token and ctx['owner_tokens'] carries them keyed
...
def scenario_field_claim(ctx):
    ...
    try:
        if ctx.get('plant') is None:
            return case.finish('inconclusive',
                               'the run publishes no plant endpoint')
        tokens = ctx.get('owner_tokens') or {}
        active = wait_for(lambda: _settled_active(ctx),
        ...
        if owner is None:
            return case.finish('inconclusive', 'the run context pins '
                               'no field-ownership token for '
                               + active)
        peer = 'standby' if active == 'active' else 'active'
        base, peer_base = ctx[active], ctx[peer]
        stream = _plant_connect(ctx)
...
# run. The field-claim case sits ahead of the parameter-tune switch:
# it needs the launched active's claim — taken at startup under the
# peer's pinned --owner-token — and its rogue-claim leg demotes and
# re-promotes the same peer, so the pair it hands on is the layout it
# found. The plant-link-loss case
...
SCENARIOS = (scenario_controller_active, scenario_standby_tracking,
             ...
             scenario_stale_freshness, scenario_field_claim,
