...
The field-fault and unavailable-fallback cases additionally open one
...
# The legs drive the same per-point inject_fault/clear_fault surface
# the field-fault case uses, against the station's served wiring: a
# backup-only fault asserts the `backup_unhealthy` annunciation while
# the healthy primary keeps the field and no failover transition
# journals; faulting the primary too drives the declared fallback with
# the alarmed engagement journaled; and clearing in the reverse order
# resumes primary control and drops the annunciation through the named
# journaled transitions. The recovery half is where the requirement's
# "no unintended output step" clause lands: every served snapshot is
# checked against the declared selection and fallback rules, so a step
# to stale, substituted, or untrusted-data-driven output fails the leg
# it was observed in.
...
def scenario_unavailable_fallback(ctx):
    ...
    try:
        ...
        hit = wait_for(annunciated, time.monotonic() + FALLBACK_DEADLINE)
        _, body = http_json('GET',
                            base + '/journal?since=' + str(floor))
        ...
        verdict = _plant_request(
            stream, {'op': 'inject_fault', 'point': points['primary'],
                     'fault': {'quality': {'bad': 'device_fault'}}})
        if verdict.get('result') != 'done':
            return case.finish('failed', 'inject_fault refused: '
                               + json.dumps(verdict)[:300])
        ...
        hit = wait_for(primary_back, time.monotonic() + FALLBACK_DEADLINE)
        ...
        if stream is not None:
            # The injected level points are the run's shared field: a
            # case that leaves them faulted poisons every later
            # scenario.
            for point in injected:
                try:
                    _plant_request(stream, {'op': 'clear_fault',
...
# drives. The field-fault and unavailable-fallback cases are
# self-contained on either role layout — including the post-recovery
# rig — and leave the rig as they found it: both drive per-point
# faults on the shared field and clear them all in teardown. The
# dcs-ctl case closes the schedule: it observes the
...
SCENARIOS = (scenario_controller_active, scenario_standby_tracking,
             ...
             scenario_field_fault, scenario_unavailable_fallback,
             scenario_dcs_ctl)
