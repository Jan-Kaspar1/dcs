...
def scenario_duty_rotation(ctx):
    ...
    try:
        ...
        if not isinstance(inflow, (int, float)) or inflow <= 0:
            return case.finish('inconclusive',
                               'rotation-nondeterministic: the deployed '
                               'dynamics declare no inflow — the level '
                               'cannot rise unopposed '
                               + json.dumps(inflow))
...
# for the cases that follow. The force-carryover case runs on the
...
# selection or the roles. The duty-rotation case sits in the same
...
# position, and fails back before the tune case's switch. The
...
SCENARIOS = (scenario_controller_active, scenario_standby_tracking,
             ...
             scenario_dead_peer_latency, scenario_force_carryover,
             scenario_backup_health, scenario_duty_rotation,
