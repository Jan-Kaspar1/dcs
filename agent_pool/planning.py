...
def planner_inventory(issues):
    """Keep current work actionable and historical dedup keys compact."""
    rows = []
    for issue in issues:
        body_text = issue.get('body') or ''
        try:
            meta = metadata(body_text)
        except (ValueError, KeyError):
            meta = {}
        row = {'number': issue['number'], 'state': issue.get('state'),
               'title': issue.get('title'), 'key': meta.get('key')}
        if issue.get('state') == 'OPEN':
            row['metadata'] = meta
            row['body'] = body_text[:3000]
            row['labels'] = [label['name'] for label in issue.get('labels', ())
                             if label['name'].startswith(('agent:', 'priority:', 'area:'))]
        rows.append(row)
    return rows


def prompt(issues, prs, output, review_ctx=None, feedback=None, allocation=None, merge_flow=None):
...
Apply the research-to-planning gate in docs/product-strategy.md. Put stable requirement IDs at the start of every product issue scope as `Requirements: ID, ID`; a pure enabler uses `Requirements: ENABLER` and names the milestone it unlocks. Research with cited evidence and customer-validation questions comes before implementation when semantics are unresolved. The current active market is water and wastewater. Follow the documented market sequence and do not create implementation tasks for later industries until their evidence and representative-plant gates are accepted. Read the current docs/plan.md instead of using historical milestone instructions from this prompt. Give each reusable library addition control, diagnostics, operator interface, simulation, and external-consumer acceptance as one complete slice. Treat Siemens APL examples as test hypotheses, not DCS compatibility requirements. Prefer finishing preserved work, verification, and current milestone prerequisites to manufacturing breadth. Delivery-platform refactors need a measured failure or feedback-time breach; merge frequency is a secondary investigation signal, not a reason by itself to create arbitrary refactors. The rolling merge comparison below warrants investigation when the decline exceeds 20% and both windows contain meaningful work; identify the measured bottleneck before proposing a delivery-platform change. Current merge flow: {json.dumps(merge_flow or {})}.
Existing issues: {json.dumps(planner_inventory(issues))}
