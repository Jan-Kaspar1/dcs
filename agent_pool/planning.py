...
Return a JSON object with ONLY an issues array by writing {output}. Each item has exactly key (stable lowercase kebab-case), title, scope, acceptance, tests (all strings), dependencies (existing GitHub issue numbers only), priority (0..3), milestone (string), group (nonempty string naming shared code/contracts that cannot run concurrently). Use distinct groups only for truly independent work. Do not duplicate existing tasks including closed issues. Aim for 6-20 ready issues if useful; zero is valid. Foundational interfaces must be established before dependent implementation. Tasks requiring an interface not yet defined should wait for the next planning cycle; do not invent dependency issue numbers. Include tests and measurable acceptance criteria. Keep orchestration/CI changes out of product tickets unless needed to repair a concrete failure.
