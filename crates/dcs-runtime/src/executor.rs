...
impl<'d> Executor<'d> {
    ...
    /// diagnoses. The `forces` section lists the active force set —
    /// each forced point with the value the image substitutes — so a
    /// monitoring consumer can badge forced points.
