...
//! and the protection-layer signal group is served data, all driven
//! over TCP through the in-process `MonitorClient` against a rig of
//! the two managed sibling kinds.
...
use dcs_core::{
    ...
    JournalEvent, PointId, PortRole, Sample, TelemetrySnapshot, Tick, Value, ValueKind,
};
...
// wired) and one `managed-bool-latching-alarm` (Bool `in`, suppression
// and out-of-service wired) — the uniform vocabulary exercised across
// kinds — plus two protection-layer field points under the declared
// `protection` signal group.
...
fn with_monitor<T>(body: impl FnOnce(&StubDriver, &MonitorClient) -> T) -> T {
    let driver = StubDriver::new(&[
        // Both conditions start clear so no alarm stands until a test
    ...
    let mut map = PointMap::new()
        ...
        // suppression condition, which is declared field wiring.
    ...
    let executor = Executor::new(&driver, map, vec![Box::new(alarm1), Box::new(alarm2)]).unwrap();
...
fn the_served_rationalization_record_serde_roundtrips() {
    with_monitor(|_driver, client| {
        ...
        let snapshot = client.snapshot().unwrap();
        for name in [ALARM1_NAME, ALARM2_NAME] {
