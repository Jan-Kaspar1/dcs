...
scenario_command_admission, scenario_controller_restart, and
scenario_plant_link_loss through their pass outcomes and the named
...
view that never reflects the produced event, a command flood whose
submissions meet dropped receipts, HTTP-layer faults, unsettled
admissions, or a bound that never fills, a restarted controller that
resumes cold, and a plant outage whose telemetry stays fresh, whose
standby promotes, whose writer claim never re-arms, or whose io_health
forgets the failures it counted."""
import json
import tempfile
import unittest
import urllib.error
