...
scenario_controller_restart, and scenario_plant_link_loss through
...
receipt, an emitted-events view that never reflects the produced
event, forced telemetry missing its Substituted stamp or forces badge,
control that ignores the force, unattributed or never-journaled
settlements, a badge that never clears, recovery that never returns to
Good, a command flood whose submissions meet dropped receipts,
HTTP-layer faults, unsettled admissions, or a bound that never fills,
a restarted controller that resumes cold, and a plant outage whose
telemetry stays fresh, whose standby promotes, whose writer claim
never re-arms, or whose io_health forgets the failures it
counted."""
import json
import tempfile
import unittest
...
class PlantLinkFeed:
    ...
    # The plant wire protocol — replaces scenarios._plant_request.
...
class PlantLinkLossTests(unittest.TestCase):
    ...
    def run_scenario(self, ctx_extra=None):
        ...
        with patch.object(scenarios, 'http_json', self.feed.http_json), \
                patch.object(scenarios, '_plant_request',
